...
void CodeGenObjectCache::notifyObjectCompiled(
  ...
  if (key_.empty()) {
    LOG(INFO) << "Insert module id " << module_id << " in CodeGenObjectCache " << this
              << " " << ObjBuffer.getBuffer().size();
    key_ = module_id;
    // The Buffer Id doesn't seem to serve a purpose in our case, and skipping it
    // can conserve memory.
    cached_obj_ = llvm::MemoryBuffer::getMemBufferCopy(
        ObjBuffer.getBuffer(), "" /*Buffer Id*/);
...
std::unique_ptr<llvm::MemoryBuffer> CodeGenObjectCache::getObject(const llvm::Module* M) {
  DCHECK(M != nullptr);
  // Return a copy of the object cache containing the compiled module with the specified
  // ID. If the ID doesn't match the one in the object cache, a warning is logged for
  // further investigation.
  const string& module_id = M->getModuleIdentifier();
  std::lock_guard<std::mutex> l(mutex_);
  if (module_id == key_) {
    LOG(INFO) << "Object for module id " << module_id << " loaded from cache. "
              << cached_obj_->getMemBufferRef().getBufferSize();
